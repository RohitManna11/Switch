...
const StreamComponent: React.FC<StreamComponentProps> = ({ livestream }) => {
  ...
  return (
    <div
      className="h-[16rem] w-[21rem] glass-container flex flex-col justify-start items-center text-white hover:bg-secondaryPurple/25 cursor-pointer"
      onClick={async () => {
    ...
    >
      <div className="relative h-[10rem] w-full">
        ...
        ></img>
        <div className="w-[3rem] py-[2px] absolute top-4 left-4 bg-primaryPurple rounded-md flex flex-row justify-center items-center text-white font-rubik font-medium text-[0.8rem]">
          LIVE
        </div>
        {livestream?.exclusive && (
          <div className="w-[6rem] py-[2px] absolute bottom-4 right-4 bg-primaryPurple rounded-md flex flex-row justify-center items-center text-white font-rubik font-medium text-[0.8rem]">
            EXCLUSIVE
          </div>
        )}
      </div>
